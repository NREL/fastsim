...
macro_rules! sim_drive_pymethods {
    () => {
        ...
        pub fn get_fs_cumu_mj_out_ach(&self) -> PyResult<Pyo3ArrayF64> {
            Ok(Pyo3ArrayF64::new(ndarrcumsum(
                &(self.fs_kw_out_ach.clone() * self.cyc.dt_s() * 1e-3),
            )))
        }
        #[getter]
        pub fn get_fc_cumu_mj_out_ach(&self) -> PyResult<Pyo3ArrayF64> {
            Ok(Pyo3ArrayF64::new(ndarrcumsum(
                &(self.fc_kw_out_ach.clone() * self.cyc.dt_s() * 1e-3),
            )))
        }
    };
}
...
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq)]
#[add_pyo3_api(sim_drive_pymethods!();)]
