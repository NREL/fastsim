    ...
    pub fn set_fc_power(&mut self, i:usize) {
        ...
        if self.fc_kw_out_ach[i] == 0.0 {
            ...
            if self.veh.fc_eff_array[np_argmax(
                &self.veh.fc_kw_out_array.map(|x| *x > min(self.fc_kw_out_ach[i], self.veh.max_fuel_conv_kw))).unwrap_or(0) - 1] != 0.0 {
                self.fc_kw_in_ach[i] = self.fc_kw_out_ach[i] / (self.veh.fc_eff_array[np_argmax(
                        &self.veh.fc_kw_out_array.map(|x| *x > min(self.fc_kw_out_ach[i], self.veh.max_fuel_conv_kw))).unwrap_or(0) - 1]);
            } else {
                self.fc_kw_in_ach[i] = 0.0
            }
        }
