...
def make_coasting_plot(
    ...
    if coast_brake_start_speed_m_per_s is not None:
        ax.plot(
            [ts[0], ts[-1]],
            [coast_brake_start_speed_m_per_s, coast_brake_start_speed_m_per_s],
            "y--",
            label="coast brake start speed",
        )
...
def make_dvdd_plot(
    ...
    for g in unique_grades:
        ...
        mask = np.logical_and(
            np.logical_and(grades == g, ks < 0.0), vavgs >= coast_to_break_speed_m__s
        )
...
class TestCoasting(unittest.TestCase):
    def setUp(self) -> None:
        ...
        if USE_RUST:
            ...
            self.ru_sim_drive = fastsim.simdrive.RustSimDrive(
                self.ru_trapz, self.ru_veh
            )
            self.ru_sim_drive_coast = fastsim.simdrive.RustSimDrive(
                self.ru_trapz, self.ru_veh
            )
            self.ru_sim_drive_coast.sim_params = set_nested_values(
                self.ru_sim_drive_coast.sim_params,
                coast_allow=True,
    ...
    def test_cycle_reported_distance_traveled_m(self):
        ...
        if USE_RUST:
            ...
            dist_m = fastsim.cycle.trapz_step_start_distance(self.trapz, idx + 1)
            self.assertAlmostEqual(
                expected_distance_m, dist_m, msg="Error in Rust version"
            )
            dds = self.trapz.calc_distance_to_next_stop_from(dist_m)
            dds_expected_m = 900 - dist_m
        ...
        if USE_PYTHON:
            ...
            for i in range(len(self.trapz.time_s)):
                ...
                if i < idx or i >= idx + n:
                    ...
                    a_expected = fastsim.cycle.accel_for_constant_jerk(
                        i - idx, accel, jerk, dt
                    )
    ...
    def test_that_we_can_coast(self):
        ...
        if USE_PYTHON:
            self.assertFalse(
                self.sim_drive.impose_coast.any(), "All impose_coast starts out False"
            )
            ...
            max_trace_miss_coast_m__s = np.absolute(
                self.trapz.mps - self.sim_drive_coast.mps_ach
            ).max()
            self.assertTrue(
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name, "junk-test-that-we-can-coast.png"
                    ),
                )
        if USE_RUST:
            self.assertFalse(
            ...
            self.assertEqual(
                17.0, self.ru_sim_drive_coast.sim_params.coast_start_speed_m_per_s
            )
            self.assertFalse(
                self.ru_sim_drive.impose_coast.any(),
            ...
            max_trace_miss_coast_m__s = np.absolute(
                self.ru_trapz.mps - self.ru_sim_drive_coast.mps_ach
            ).max()
            self.assertTrue(
                max_trace_miss_coast_m__s > 1.0,
                f"Max trace miss: {max_trace_miss_coast_m__s} m/s",
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    title="Test That We Can Coast",
                    save_file=os.path.join(
                        OUTPUT_DIR.name, "junk-test-that-we-can-coast-rust.png"
                    ),
                )
        if DO_TESTS:
            OUTPUT_DIR.cleanup()
    ...
    def test_eco_approach_modeling(self):
        ...
        if USE_PYTHON:
            ...
            self.assertFalse(
                self.sim_drive_coast.impose_coast.all(),
                "Assert we are not always in coast",
            )
            self.assertTrue(
                self.sim_drive_coast.impose_coast.any(),
                "Assert we are at least sometimes in coast",
            )
            max_trace_miss_coast_m__s = np.absolute(
                self.trapz.mps - self.sim_drive_coast.mps_ach
            ).max()
            self.assertTrue(
                max_trace_miss_coast_m__s > 1.0,
                "Assert we deviate from the shadow trace",
            )
            self.assertTrue(
                self.sim_drive_coast.mph_ach.max() > 20.0,
                "Assert we at least reach 20 mph",
            )
            self.assertAlmostEqual(
        ...
        if USE_RUST:
            ...
            self.assertFalse(
                self.ru_sim_drive_coast.impose_coast.all(),
                "Assert we are not always in coast",
            )
            self.assertTrue(
                self.ru_sim_drive_coast.impose_coast.any(),
                "Assert we are at least sometimes in coast",
            )
            max_trace_miss_coast_m__s = np.absolute(
                self.ru_trapz.mps - self.ru_sim_drive_coast.mps_ach
            ...
            self.assertTrue(
                self.ru_sim_drive_coast.mph_ach.max() > 20.0,
                "Assert we at least reach 20 mph",
            )
            self.assertAlmostEqual(
                np.array(self.ru_trapz.dist_m).sum(),
                self.ru_sim_drive_coast.dist_m.sum(),
                msg="Assert the end distances are equal\n"
                + f"Got {np.array(self.ru_trapz.dist_m).sum()} m and {self.ru_sim_drive_coast.dist_m.sum()} m",
            )
    ...
    def test_that_final_speed_of_cycle_modification_matches_trajectory_calcs(self):
        ...
        if USE_PYTHON:
            ...
            dtbi0 = dts0 - dtb
            jerk_m__s3, accel_m__s2 = fastsim.cycle.calc_constant_jerk_trajectory(
                n, d0, v0, d0 + dtbi0, brake_start_speed_m__s, dt
            )
            final_speed_m__s = self.trapz.modify_by_const_jerk_trajectory(
                idx, n, jerk_m__s3, accel_m__s2
            )
        ...
        if USE_RUST:
            ...
            dtbi0 = dts0 - dtb
            jerk_m__s3, accel_m__s2 = fsr.calc_constant_jerk_trajectory(
                n, d0, v0, d0 + dtbi0, brake_start_speed_m__s, dt
            )
        ...
        if USE_PYTHON:
            ...
            const_spd_cyc = fastsim.cycle.Cycle.from_dict(
                fastsim.cycle.resample(
                    fastsim.cycle.make_cycle([0.0, 20.0], [0.0, 20.0]), new_dt=1.0
                )
            )
            expected_dist_m = 200.0  # 0.5 * 20m/s x 20s = 200m
            self.assertAlmostEqual(
                expected_dist_m, fastsim.cycle.trapz_step_distances(const_spd_cyc).sum()
            )
    ...
    def test_brake_trajectory(self):
        ...
        if USE_PYTHON:
            ...
            self.assertAlmostEqual(
                v0 + brake_accel_m__s2 * 10 * dt, trapz.mps[idx + 10]
            )
    ...
    def test_logic_to_enter_eco_approach_automatically(self):
        ...
        if USE_PYTHON:
            ...
            sd.sim_drive()
            self.assertTrue(
                sd.impose_coast.any(), msg="Coast should initiate automatically"
            )
            if DO_TESTS:
                make_coasting_plot(
                    sd.cyc0,
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test-logic-to-enter-eco-approach-automatically-1.png",
                    ),
                )
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test-logic-to-enter-eco-approach-automatically-2.png",
                    ),
                )
                make_dvdd_plot(
                    sd.cyc,
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test-logic-to-enter-eco-approach-automatically-3-dvdd.png",
                    ),
        ...
        if USE_RUST:
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test-logic-to-enter-eco-approach-automatically-1-rust.png",
                    ),
                )
            trapz2 = fastsim.cycle.Cycle.from_dict(
                fastsim.cycle.resample(
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test-logic-to-enter-eco-approach-automatically-2-rust.png",
                    ),
                )
                make_dvdd_plot(
                    sd.cyc,
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test-logic-to-enter-eco-approach-automatically-3-dvdd-rust.png",
                    ),
                    coast_to_break_speed_m__s=11.0,
                )
        if DO_TESTS:
        ...
        if USE_PYTHON:
            ...
            if DO_TESTS:
                ...
                def dvdd(vavg, grade):
                    ...
                    M = veh.veh_kg
                    rho_CDFA = (
                        sd.props.air_density_kg_per_m3
                        * veh.frontal_area_m2
                        * veh.drag_coef
                    )
                    return (g / vavg) * (
                        np.sin(atan_grade) + veh.wheel_rr_coef * np.cos(atan_grade)
                ...
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_works_going_uphill-trace.png",
                    ),
                )
                make_dvdd_plot(
                    sd.cyc,
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_works_going_uphill-dvdd.png",
                    ),
        ...
        if USE_RUST:
            ...
            if DO_TESTS:
                ...
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_works_going_uphill-trace-rust.png",
                    ),
                )
                make_dvdd_plot(
                    sd.cyc,
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_works_going_uphill-dvdd-rust.png",
                    ),
                    coast_to_break_speed_m__s=5.0,
                    additional_xs=vavgs,
                    additional_ys=ks,
        ...
        if USE_PYTHON:
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_logic_works_going_uphill-trace-vehicle-5.png",
                    ),
                )
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_logic_works_going_uphill-trace-vehicle-1.png",
                    ),
                )
        ...
        if USE_RUST:
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_logic_works_going_uphill-trace-vehicle-5-rust.png",
                    ),
                )
            # assert we have grade set correctly
            self.assertTrue((sd.cyc0.grade == grade).all())
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_logic_works_going_uphill-trace-vehicle-1-rust.png",
                    ),
                )
            # TODO: should we use sd.cyc.dist_m or fastsim.cycle.trapz_step_distances().sum() for sd.cyc below?
            self.assertAlmostEqual(
        ...
        if USE_PYTHON:
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_logic_works_going_downhill-trace-vehicle-5.png",
                    ),
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_logic_works_going_downhill-trace-vehicle-1.png",
                    ),
                    coast_brake_start_speed_m_per_s=sd.sim_params.coast_brake_start_speed_m_per_s,
                )
            # TODO: should we use sd.cyc.dist_m or fastsim.cycle.trapz_step_distances().sum() for sd.cyc below?
            self.assertAlmostEqual(
                fastsim.cycle.trapz_step_distances(sd.cyc0).sum(), sd.cyc.dist_m.sum()
            )
        if USE_RUST:
            grade = -0.0025
            trapz = fastsim.cycle.Cycle.from_dict(
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_logic_works_going_downhill-trace-vehicle-5-rust.png",
                    ),
                    coast_brake_start_speed_m_per_s=sd.sim_params.coast_brake_start_speed_m_per_s,
                )
            # assert we have grade set correctly
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_logic_works_going_downhill-trace-vehicle-1-rust.png",
                    ),
                    coast_brake_start_speed_m_per_s=sd.sim_params.coast_brake_start_speed_m_per_s,
                )
            # TODO: should we use sd.cyc.dist_m or fastsim.cycle.trapz_step_distances().sum() for sd.cyc below?
            self.assertAlmostEqual(
                fastsim.cycle.trapz_step_distances(sd.cyc0).sum(), np.array(sd.cyc.dist_m).sum()
            )
        if DO_TESTS:
            OUTPUT_DIR.cleanup()
    ...
    def test_that_coasting_works_with_multiple_stops_and_grades(self):
        ...
        if USE_PYTHON:
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_works_with_multiple_stops_and_grades-veh5.png",
                    ),
                    coast_brake_start_speed_m_per_s=sd.sim_params.coast_brake_start_speed_m_per_s,
                )
            # assert we have grade set correctly
        ...
        if USE_RUST:
            ...
            veh = fastsim.vehicle.Vehicle.from_vehdb(5).to_rust()
            cyc = fastsim.cycle.Cycle.from_dict(
                fastsim.cycle.concat([c1, c2])
            ).to_rust()
            ...
            if DO_TESTS:
                make_coasting_plot(
                    ...
                    save_file=os.path.join(
                        OUTPUT_DIR.name,
                        "junk-test_that_coasting_works_with_multiple_stops_and_grades-veh5-rust.png",
                    ),
                    coast_brake_start_speed_m_per_s=sd.sim_params.coast_brake_start_speed_m_per_s,
                )
            # assert we have grade set correctly
