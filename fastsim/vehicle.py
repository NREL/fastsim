...
def copy_vehicle(veh: Vehicle, return_type: str = None, deep: bool = True):
    ...
    for key in keys_and_types.keys():
        if (
            ...
            if hasattr(veh.__getattribute__(key), 'to_list'):
                attr_val = np.array(veh.__getattribute__(key).to_list())
            else:
                attr_val = veh.__getattribute__(key)
            veh_dict[key] = copy.deepcopy(attr_val) if deep else attr_val
