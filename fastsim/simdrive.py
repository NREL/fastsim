...
def copy_sim_drive(sd_in, use_jit=False):
    """
    Given fully solved instance of SimDriveClassic or SimDriveJit, returns copy.  

    Arguments:
    ----------
    sd_in: solved SimDriveClassic or SimDriveJit
    use_jit: Boolean, if true, return SimDriveJit
    """
    from . import simdrivejit
    ...
    if use_jit:
        sd_out = SimDriveJit(
            ...
            vehicle.Vehicle(1, verbose=False).get_numba_veh(), 
        )
    else:
        sd_out = SimDriveClassic(
            cycle.Cycle('udds'), 
    ...
    for keytup in simdrivejit.sim_drive_spec:
        ...
        print(key)
        if key == 'cyc':
            sd_out.cyc = cycle.copy_cycle(sd_in.cyc, use_jit=use_jit)

        elif key == 'cyc0':
            sd_out.cyc0 = cycle.copy_cycle(sd_in.cyc0, use_jit=use_jit)

        elif key == 'veh':
            sd_out.veh = vehicle.copy_vehicle(sd_in.veh, use_jit=use_jit)
    
        else:
            sd_out.__setattr__(key, sd_in.__getattribute__(key))
