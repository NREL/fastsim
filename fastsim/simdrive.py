...
class SimDrive(object):
    ...
    def _calc_distance_to_stop_coast_v2(self, i):
        ...
        while v > v_brake and v >= 0.0 and d <= d_max and i < MAX_ITER:
            gr = unique_grade if unique_grade is not None else np.interp(
                d, distances_m, grade_by_distance)
...
rust_list_types = [fsr.Pyo3ArrayBool, fsr.Pyo3ArrayF64, fsr.Pyo3ArrayU32]
python_list_types = [list, np.ndarray]
        ...
        if k in ['cyc', 'cyc0']:
            ...
            if not sim_params_equal(a_val, b_val):
                if verbose:
                    print(f"unequal at key {k}: {a_val} != {b_val}")
                return False
        elif type(a_val) in python_list_types and type(b_val) in python_list_types:
            if not (np.array(a_val) == np.array(b_val)).all():
                if verbose:
                    print(f"unequal at key {k}: {a_val} != {b_val}")
                return False
        elif type(a_val) in rust_list_types and type(b_val) in rust_list_types:
            if not a_val.to_list() == b_val.to_list():
                if verbose:
                    print(f"unequal at key {k}: {a_val} != {b_val}")
                return False
        elif type(a_val) in rust_list_types:
            if not (a_val.to_list() == b_val).all():
                if verbose:
                    print(f"unequal at key {k}: {a_val} != {b_val}")
                return False
        elif type(b_val) in rust_list_types:
            if not (a_val == b_val.to_list()).all():
                if verbose:
                    print(f"unequal at key {k}: {a_val} != {b_val}")
                return False
        elif a_val != b_val:
