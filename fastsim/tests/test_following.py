...
class TestFollowing(unittest.TestCase):
    ...
    def test_sweeping_idm_parameters(self):
        ...
        if DO_PLOTS:
            ...
            udds_dict = fastsim.cycle.Cycle('udds').get_cyc_dict()
            udds = fastsim.cycle.Cycle(
            ...
            mask = udds.cycDistMeters_v2 > 0
            avg_speed_when_moving_m__s = udds.cycDistMeters_v2[mask].sum() / udds.dt_s[mask].sum()
            # DESIGN VARIABLES
            ...
            if not pl.Path(datafile_path).exists():
                ...
                for pt_type in veh_pt_types:
                    veh = fastsim.vehicle.Vehicle(id_by_pt_type[pt_type], verbose=False).get_numba_veh()
                    ...
                    if sd.mpgge > 0.0:
                        ...
                        gge__mi = sd.battery_kWh_per_mi / sd.props.kWhPerGGE
