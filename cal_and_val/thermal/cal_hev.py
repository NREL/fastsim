...
cyc_folder_path = (
    Path(__file__).parent / "dyno_test_data/2021 Hyundai Sonata Hybrid/Extended Datasets"
)
...
def veh_init(cyc_file_stem: str, dfs: Dict[str, pd.DataFrame]) -> fsim.Vehicle:
    ...
    assert 0 < vd["pt_type"][pt_type_var]["res"]["state"]["soc"] < 1, (
        "\ninit soc: {}\nhead: {}".format(
            vd["pt_type"][pt_type_var]["res"]["state"]["soc"],
            dfs[cyc_file_stem]["HVBatt_SOC_high_precision_PCAN__per"].head(),
        )
    )
    ...
    vd["pt_type"][pt_type_var]["fc"]["thrml"]["FuelConverterThermal"]["state"][
        "temperature_kelvin"
    ...
    vd["hvac"]["LumpedCabin"]["te_set_kelvin"] = (
...
def new_cab_shell_htc_w_per_m2_k(sd_dict, new_val) -> Dict:
    sd_dict["veh"]["cabin"]["LumpedCabin"]["cab_shell_htc_to_amb_watts_per_square_meter_kelvin"] = (
        new_val
    )
    return sd_dict
...
def new_cab_htc_to_amb_stop_w_per_m2_k(sd_dict, new_val) -> Dict:
    sd_dict["veh"]["cabin"]["LumpedCabin"]["cab_htc_to_amb_stop_watts_per_square_meter_kelvin"] = (
        new_val
    )
    return sd_dict
...
def new_speed_soc_disch_buffer_m_per_s(sd_dict, new_val) -> Dict:
    sd_dict["veh"]["pt_type"][pt_type_var]["pt_cntrl"]["RGWDB"][
        "speed_soc_disch_buffer_meters_per_second"
    ] = new_val
    return sd_dict


def new_speed_soc_disch_buffer_coeff(sd_dict, new_val) -> Dict:
    sd_dict["veh"]["pt_type"][pt_type_var]["pt_cntrl"]["RGWDB"]["speed_soc_disch_buffer_coeff"] = (
        new_val
    )
    return sd_dict
...
def new_speed_soc_fc_on_buffer_m_per_s(sd_dict, new_val) -> Dict:
    sd_dict["veh"]["pt_type"][pt_type_var]["pt_cntrl"]["RGWDB"][
        "speed_soc_fc_on_buffer_meters_per_second"
    ] = new_val
    return sd_dict


def new_speed_soc_fc_on_buffer_coeff(sd_dict, new_val) -> Dict:
    sd_dict["veh"]["pt_type"][pt_type_var]["pt_cntrl"]["RGWDB"]["speed_soc_fc_on_buffer_coeff"] = (
        new_val
    )
    return sd_dict
...
def new_frac_pwr_demand_fc_forced_on(sd_dict, new_val) -> Dict:
    sd_dict["veh"]["pt_type"][pt_type_var]["pt_cntrl"]["RGWDB"]["frac_pwr_demand_fc_forced_on"] = (
        new_val
    )
    return sd_dict
...
def new_fc_thrml_heat_capacitance_j_per_k(sd_dict, new_val) -> Dict:
    sd_dict["veh"]["pt_type"][pt_type_var]["fc"]["thrml"]["FuelConverterThermal"][
...
def new_fc_thrml_fc_eff_model_Exponential_offset(sd_dict, new_val) -> Dict:
    sd_dict["veh"]["pt_type"][pt_type_var]["fc"]["thrml"]["FuelConverterThermal"]["fc_eff_model"][
        "Exponential"
    ]["offset"] = new_val
    return sd_dict
...
def get_mod_cab_temp_celsius(sd_dict):
    return (
        np.array(sd_dict["veh"]["cabin"]["LumpedCabin"]["history"]["temperature_kelvin"])
        - celsius_to_kelvin_offset
    )
...
def get_mod_energy_fuel_megajoules(sd_dict):
    return (
        np.array(sd_dict["veh"]["pt_type"][pt_type_var]["fc"]["history"]["energy_fuel_joules"])
        / 1e6
    )
...
def get_mod_pwr_hvac_kw(sd_dict):
    return (
        np.array(sd_dict["veh"]["hvac"]["LumpedCabin"]["history"]["pwr_aux_for_hvac_watts"]) / 1e3
    )
...
def get_fc_temp_too_hot(sd_dict):
    te_fc_deg_c = (
        sd_dict["veh"]["pt_type"][pt_type_var]["fc"]["thrml"]["FuelConverterThermal"]["state"][
            "temperature_kelvin"
        ]
        - celsius_to_kelvin_offset
    )
...
cal_mod_obj = pymoo_api.ModelObjectives(
    ...
    obj_fns=(
        ...
        (get_mod_energy_fuel_megajoules, get_exp_energy_fuel_megajoules),
        (get_mod_pwr_fuel_kw, get_exp_pwr_fuel_kw),
        (get_mod_cab_temp_celsius, get_exp_cab_temp_celsius),
        (get_mod_fc_temp_celsius, get_exp_fc_temp_celsius),
        (get_mod_speed_m_per_s, get_exp_speed_m_per_s),
        (get_mod_pwr_hvac_kw, get_exp_pwr_hvac_kw),
    ),
    ...
    constr_fns=(get_fc_temp_too_hot,),
    verbose=False,
)
...
def perturb_params(pos_perturb_dec: float = 0.05, neg_perturb_dec: float = 0.1):
    ...
    baseline_params_and_bounds = [
        ...
        (
            veh_dict["pt_type"]["HybridElectricVehicle"]["pt_cntrl"]["RGWDB"][
                "speed_soc_disch_buffer_meters_per_second"
            ],
            None,
        ),
        ...
        (
            veh_dict["pt_type"]["HybridElectricVehicle"]["fc"]["thrml"]["FuelConverterThermal"][
                "heat_capacitance_joules_per_kelvin"
            ],
            None,
        ),
